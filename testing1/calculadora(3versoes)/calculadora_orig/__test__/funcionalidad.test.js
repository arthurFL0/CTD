...
describe('Validate function Init', () => {
    ...
    test('Press button /', () =>{
        ...
        expect(validateResult.innerHTML).toBe("");
        expect(ret[2]).toBe("/");
        validateResult.innerHTML = value; //default value to Re
    test('Press button =', ()=>{
        const validateElement_igual = document.getElementById('igual')
        const validateResult = document.getElementById(id)
        expect(validateElement_igual).toBeDefined();        
        expect(validateElement_igual.innerHTML).toBe("=");  
        expect(validateElement_igual.innerHTML).toBe("10"); 
        limpiar();
        init();
        validateElement_igual.onclick();
        const ret = setVars()
        expect(validateElement_igual.innerHTML).toBe("");
        expect(ret[2]).toBe("=");
